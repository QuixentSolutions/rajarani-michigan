...
function Header() {
  ...
  // --- FIX APPLIED HERE ---
  // Default delivery modes are provided to ensure the options always show up.
  // The API call below will still try to update them with values from your database.
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (!dbResponse.ok) {
          // If the API fails, we'll just use the default modes set above.
          console.error("Failed to fetch settings from the server.");
          return;
        }
        ...
        // Only update if the API returns valid modes, otherwise keep the default.
        if (result.length > 0) {
