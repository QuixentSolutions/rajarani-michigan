...
// GET /delivery-partner-sales/reports/:period  — daily (30d) | weekly (8w) | monthly (12m)
router.get("/reports/:period", async (req, res) => {
  try {
    ...
    const raw = await require("../models/deliveryPartnerSales").aggregate([
    ...
    res.json(raw.map((r) => ({ label: labelFn(r), gross: r.gross, commission: r.commission, net: r.net, total: r.total, count: r.count })));
  } catch (err) {
