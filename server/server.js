  ...
  try {
    ...
    for (const section of sections) {
      ...
      section.items.forEach((item, index) => {
        if (!item._id) {
          console.log(`Fixing item ${index}: ${item.name} - adding new ObjectId`)
    ...
    console.log(`Fix complete: ${totalItemsFixed} items fixed in ${sectionsProcessed} sections`)
    res.json({
    ...
    res.status(500).json({ message: "Failed to fix items", error: error.message })
  }
})

// Paginated get routes with search
const createPaginatedGetRoute = (model, searchFields = []) => async (req, res) => {
  try {
    ...
    const items = await model.find(query).sort({ _id: -1 }).skip(skip).limit(limit)
...
app.get("/api/admin/registrations", createPaginatedGetRoute(Registration, ['name', 'email']))
app.get("/api/admin/orders", createPaginatedGetRoute(Order, ['orderId', 'email', 'mobileNumber']))
...
app.get("/api/admin/menu", async (req, res) => {
  try {
    ...
    const totalPages = Math.ceil(totalItems / limit) || 1
    const sections = await MenuSection.find(query).sort({ createdDate: 1 }).skip(skip).limit(limit)
...
app.post("/api/admin/menu/:id/items", async (req, res) => {
  try {
    ...
    console.log("Adding item request:", { sectionId, name, newPrice, oldPrice })
...
app.put("/api/admin/menu/:id/items/:itemId", async (req, res) => {
  try {
    ...
    console.log("Update request:", { sectionId, itemId, name, newPrice, oldPrice })
    ...
    if (!item) {
      console.log("Available item IDs:", section.items.map(i => i._id))
    ...
    if (oldPrice !== undefined) item.oldPrice = oldPrice ? oldPrice.toString() : null
...
app.post("/api/admin/menu", async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to create menu section", error: error.message })
  }
})
