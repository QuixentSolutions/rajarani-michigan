...
const AdminDashboard = ({ onLogout }) => {
  ...
  const fixExistingItems = async () => {
    if (!window.confirm("This will fix all existing menu items to work with CRUD operations. Continue?")) {
      return
    }
    try {
      setError("")
      setSuccess("Fixing existing items...")
      const response = await fetch(`${API_BASE_URL}/api/admin/fix-existing-items`, {
        method: "POST",
        headers: {
      ...
      setSuccess(`Successfully fixed ${result.totalItemsFixed} items in ${result.sectionsProcessed} sections`)
  ...
  const createFetchFunction = useCallback(
    (setter, type) =>
      async (page = 1, searchQuery = "") => {
        try {
          setError("")
          const url = `${API_BASE_URL}/api/admin/${type}?page=${page}${searchQuery ? `&search=${searchQuery}` : ""}`
  ...
  const refreshSelectedSectionItems = async (sectionToRefresh) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/admin/menu/${sectionToRefresh._id}`, {
          ...
          <div className="modal-body">
            {modalType === "manage-items" && selectedSection && (
              <div className="items-management">
                ...
                <div style={{ margin: "10px 0", padding: "10px", background: "#f8f9fa", borderRadius: "5px", fontSize: "12px" }}>
                  <strong>Debug Info:</strong>
                  <div>Section ID: {selectedSection._id}</div>
                  <div>Section has {selectedSection.items?.length || 0} items</div>
                  {selectedSection.items?.length > 0 && (
                    <div>
                      Item IDs: {selectedSection.items.map(item => `${item.name}: ${item._id || 'NO_ID'}`).join(', ')}
                    </div>
                  )}
                </div>
                          ...
                          {editingMenuItem === item._id ? (
                            <div className="edit-form">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  setEditingItemData({ ...editingItemData, newPrice: e.target.value })
                                }}
                                className="form-input"
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  setEditingItemData({ ...editingItemData, oldPrice: e.target.value })
                                }}
                                className="form-input"
                            ...
                            <div className="item-display">
                              ...
                              <div className="item-id-debug" style={{ fontSize: "10px", color: hasValidId ? "#28a745" : "#dc3545" }}>
                                ID: {item._id || 'NO_ID'} {!hasValidId && '⚠️ Invalid ID'}
                              </div>
                              <div className="item-actions">
                                <button 
                                  ...
                                  className="btn-edit"
                                  disabled={!hasValidId}
                                  title={!hasValidId ? "Item has no valid ID - use Fix Existing Items button" : "Edit item"}
                                >
                                  Edit
                                </button>
                                <button 
                                  ...
                                  disabled={!hasValidId}
                                  title={!hasValidId ? "Item has no valid ID - use Fix Existing Items button" : "Delete item"}
                                >
                                  Delete
                                </button>
                              </div>
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(`Server responded with ${response.status}: ${errorData.message || 'Failed to update status'}`)
      }

      const result = await response.json()
      setSuccess(`Order ${orderId} status updated to ${result.order.status.toUpperCase()}!`)
    ...
    <div className="admin-dashboard">
      ...
      {activeTab === "registrations" && (
        <div className="section-card">
          ...
          {renderPagination(registrations, (page) => fetchRegistrations(page, searchRegistrationQuery))}
        </div>
      )}
                ...
                {orders.items.length > 0 ? (
                  orders.items.map((order) => (
                    <tr key={order._id}>
                      ...
                      <td>
                        <span className={getStatusBadgeClass(order.orderMode, "mode")}>
                      ...
                      <td>
                        {editingOrderId === order._id ? (
                          ...
                          <span 
                            className={getStatusBadgeClass(order.status, "order")}
      ...
      {activeTab === "menu" && (
        <div className="section-card">
                ...
                {menuData.items.length > 0 ? (
                  menuData.items.map((section, index) => (
                    <tr key={section._id || section.id || index}>
                      <td>
                        <strong>{section.title || section.name || `Section ${index + 1}`}</strong>
                      </td>
                      <td>
                        <div className="color-display">
                          <div
                            className="color-circle"
                            style={{ backgroundColor: section.color || "#FFA500" }}
                          ></div>
                          <span>{section.color || "#FFA500"}</span>
                        </div>
                      </td>
                      <td>
                        <strong>{section.itemCount || section.items?.length || 0}</strong>
                      </td>
                      <td>
          ...
          {renderPagination(menuData, (page) => fetchMenuData(page, searchMenuQuery))}
        </div>
      )}
